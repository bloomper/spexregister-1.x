...
var DatePicker = {
  ...
  days: 'Sön,Mån,Tis,Ons,Tor,Fre,Lör'.split(','),
  ...
  changeCalendar: function (id, offset) {
    ...
    var temp_month = d2.getMonth() + 1;
    if (temp_month < 10) {
      temp_month = "0" + temp_month;
    }
    var temp_day = d2.getDate();
    if (temp_day < 10) {
      temp_day = "0" + temp_day;
    }
    
    ansi_date = d2.getFullYear() + '-' + temp_month + '-' + temp_day;
  ...
  setDate: function (id, ansiDate) {
    ...
    this.findLinkElement(id).innerHTML = ansiDate;
  },
  ...
  ansiDateFromDate: function(date) {
    ...
    alert( date.getFullYear() + '-' + temp_month + '-' + temp_day );
  },
  ...
  selectMonth: function (id, n) {
    ...
    this.setDate(id, d2.getFullYear() + '-' + temp_month + '-' + temp_day )
    this.writeCalendar(id)
  },
  ...
  selectYear: function (id, n) {
    ...
    this.setDate(id, n + '-' + temp_month + '-' + temp_day )
    this.writeCalendar(id)
  },
  ...
  writeCalendar: function (id) {
    ...
    if (this.showCancelLink) {
      o += '<tfoot><tr><td colspan="7"><div class="cancel_butt"><a href="#" onclick="DatePicker.cancel(\''+id+'\')">[x] Tillbaka</a></div></td></tr></tfoot>';
    }
    ...
    for(rows = 1; rows < 7 && (this.constantHeight || day < lastDateOfMonth); rows++) {
      ...
      for(var day_num = 0; day_num < this.days.length; day_num++) {
        ...
        if ((translated_day >= firstWeekday || day > 1) && (day <= lastDateOfMonth) ) {
          ...
          args = [id, (date.getFullYear() + '-' + temp_month + '-' + temp_day)];
          var selected_date = this.getSelectedDate(id);
          if(selected_date.getFullYear() == date.getFullYear() && selected_date.getMonth() == date.getMonth() && selected_date.getDate() == day) {
            style = (this.selectMonth ? 'style="width: ' + this.largeCellSize + 'px"' : '');
            o +=
              '<td ' + style  + '>' + // link : each day
              "<a href=\"#\" style='color:red' onclick=\"DatePicker.pickDate('" + args.join("','") + "'); return false;\">" + day + '</a>' +
              '</td>';
          } else {
            style = (this.selectMonth ? 'style="width: ' + this.largeCellSize + 'px"' : '');
            o +=
