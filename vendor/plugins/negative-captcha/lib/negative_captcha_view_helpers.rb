module NegativeCaptchaViewHelpers
  ...
  def negative_password_field_tag(negative_captcha, field, options={})
    [
      password_field_tag(negative_captcha.fields[field], negative_captcha.values[field], options.merge(:tabindex => '1')),
      "<div style='position: absolute; left: -2000px;'>" + password_field_tag(field, '', :tabindex => '999', :autocomplete => 'off') + "</div>"
    ].join
  end
