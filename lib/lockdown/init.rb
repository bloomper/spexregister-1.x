...
# set_permission(:spexare_myself).with_controller(:spexare).to_model(:spexare).where(:editable_by).includes(:current_user_id).and_controller(:relationships).and_controller(:memberships).and_controller(:activities)
