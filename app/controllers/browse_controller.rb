class BrowseController < BaseController
  ...
  before_filter :is_admin_or_owner_xhr?, :only => [:new_link_item, :add_link_item, :edit_link_item, :update_link_item, :cancel_link_item, :destroy_link_item, :move_up_link_item, :move_down_link_item]
  ...
  private
    def is_admin_or_owner?
      ...
      if(!user_item.nil? && user_item.is_in_role?(RoleItem::ADMIN))
        return true
      else
        if(!params[:id].blank? && !user_item.nil? && user_item.spexare_item.id == params[:id].to_i)
          return true
        else
    ...
    def is_admin_or_owner_xhr?
      user_item = get_user_from_session
      if(!user_item.nil? && user_item.is_in_role?(RoleItem::ADMIN))
        return true
      else
        if(!params[:spexare_item_id].blank? && !user_item.nil? && user_item.spexare_item.id == params[:spexare_item_id].to_i)
          return true
        else
          send_back_error('Du har ej rättigheter till att utföra detta.')
        end
      end
    end
